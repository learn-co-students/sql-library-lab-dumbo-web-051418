...
def select_series_title_with_most_human_characters
  "SELECT title FROM (SELECT series.title AS title, characters.species AS species FROM characters LEFT JOIN series ON series.id = characters.series_id) GROUP BY title HAVING (species = 'human') ORDER BY COUNT(*) DESC;"
end
